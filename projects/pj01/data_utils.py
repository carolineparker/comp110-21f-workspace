...
def head(data_dictionary: dict[str, list[str]], rows: int) -> dict[str, list[str]]:
    ...
    for key in data_dictionary:
        empty_list: list[str] = []
        i: int = 0
        if rows > len(data_dictionary[key]):
...
def filter_data(data_dict: dict[str, list[str]]) -> dict[str, list[str]]:
    ...
    for key in data_dict:
        ...
        while i < len(data_dict[key]):
            if data_dict[key][i] == 'Yes':
                filtered_dict[key].append(data_dict[key][i])
            if data_dict[key][i] == 'No':
                filtered_dict[key].append(data_dict[key][i])
            i += 1
